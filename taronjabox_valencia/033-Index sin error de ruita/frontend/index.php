<!DOCTYPE html>
<html lang="es">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>TaronjaBox Valencia</title>
    <link rel="stylesheet" href="assets/css/style.css"> <!-- Enlace al CSS -->
    <link rel="stylesheet" href="views/destacados/destacados.css">
</head>

<body>
    <?php include 'views/includes/header.php'; ?>
    <main>
        <h1>Bienvenido a TaronjaBox Valencia</h1>
        <p>Explora nuestras secciones usando el menú de navegación.</p>

        <!-- Sección de destacados -->
        <section id="destacados-section">
            <h2>Destacados</h2>
            <div id="destacados-container">
                <p>Cargando destacados...</p>
            </div>
        </section>
    </main>

    <!-- Enlace al JavaScript -->
    <script src="views/destacados/destacados.js"></script>

    <?php include 'views/includes/footer.php'; ?>
</body>

</html>