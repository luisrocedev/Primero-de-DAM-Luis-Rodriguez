document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentPhotoUrl = ''; // Para almacenar la imagen actual durante la edición
        ...
        try {
            ...
            trainersContainer.innerHTML = trainers.map(trainer => `
                <div class="trainer">
                    ...
                    <p>Especialidad: ${trainer.especialidad}</p>
                    <p>Descripción: ${trainer.descripcion}</p>
    ...
    window.editTrainer = (id, nombre, especialidad, descripcion, foto_url) => {
        document.getElementById('id').value = id; // Asignar el ID del entrenador al formulario
        document.getElementById('nombre').value = nombre;
        document.getElementById('especialidad').value = especialidad;
        document.getElementById('descripcion').value = descripcion;
        currentPhotoUrl = foto_url; // Guardar la foto actual
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (editMode && !formData.get('foto')) {
            formData.append('foto_url', currentPhotoUrl); // Conservar la foto actual si no se sube una nueva
        }

        const method = editMode ? 'POST' : 'POST'; // Ambos casos usan POST, diferenciamos con _method
        if (editMode) {
        ...
        try {
            ...
            if (result.success) {
                ...
                editMode = false;
                currentPhotoUrl = ''; // Reseteamos la imagen actual
                loadTrainers(); // Recargamos la lista de entrenadores
            } else {
