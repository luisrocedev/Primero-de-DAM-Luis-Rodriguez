document.querySelector("button").onclick = function(){                                  // Cuando en el boton hago click
    ...
    let contrasena = document.querySelector("#contrasena").value                        // Atrapo el valor de la contrasena
    fetch("http://127.0.0.1:5000/login?usuario="+usuario+"&contrasena="+contrasena)     // Envio una peticion por fetch a flask
    ...
    .then(function(datos){                                                              // Cuando eso haya ocurrido
        if(datos.resultado == ['verdadero']){                                           // Si el resultado dice que es verdadero
            ...
            window.location = window.location                                           // Recargo la pagina
        }
    })
}