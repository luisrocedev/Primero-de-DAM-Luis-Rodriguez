...
class EntrenadoresController {
    ...
    public function createEntrenador($nombre, $especialidad, $descripcion, $foto_url) {     // Función para crear un nuevo entrenador
        global $conn;                                                                       // Utiliza la conexión global
        if (!$this->getConnectionStatus()) {                                                // Verificar que la conexión se haya establecido, es decir,si no hay conexión
            die("Conexión fallida a la base de datos.");                                    // Termina la ejecución con un mensaje de error
        }
        $query = $conn->prepare("INSERT INTO entrenadores (nombre, especialidad, descripcion, foto_url) VALUES (:nombre, :especialidad, :descripcion, :foto_url)"); // Consulta SQL para insertar un nuevo entrenador en la base de datos
