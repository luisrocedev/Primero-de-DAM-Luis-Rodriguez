...
class PrecioController {
    ...
    public function handleRequest() {
        ...
        switch ($method) {
            ...
            case 'PUT': // Editar un precio existente
                parse_str(file_get_contents("php://input"), $_PUT);
                $id = $_PUT['id'] ?? 0;
                $nombre_plan = $_PUT['nombre_plan'] ?? '';
                $descripcion = $_PUT['descripcion'] ?? '';
                $precio = $_PUT['precio'] ?? 0.0;
                echo json_encode(['success' => $this->precio->updatePrice($id, $nombre_plan, $descripcion, $precio)]);
                break;
